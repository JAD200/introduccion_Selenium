...
##      Installs the chrome driver (log_level=0: Disables teh log.info for the console)
    ...
    def test_search_text_field_by_class_name(self):
        search_field = self.driver.find_element_by_class_name('input-box')

##      Verifies if button search is enabled
    def test_search_button_enabled(self):
        button = self.driver.find_elements_by_class_name('button search-button')

##      Verifies if number of images in promos class equals 3
    def test_count_of_promo_banner_images(self):
        banner_list = self.driver.find_element_by_class_name('promos')
        banners = banner_list.find_elements_by_tag_name('img')
        self.assertEqual(3, len(banners))

##!     Tests an image by the Xpath (not recommended)
    def test_vip_promo(self):
        vip_promo = self.driver.find_element_by_xpath('//*[@id="top"]/body/div/div[2]/div[2]/div/div/div[2]/div[1]/ul/li[4]/a/img')

##      Tests the shopping cart icon by the css selector
